...
def change_input(filename, frac, seed=None, time=20000):
    ...
    frac_Lp = round(frac * frac_LT, 7)
    frac_w = round(1 - frac_Lp, 7)

    params = {'Box': "32 32 32\t1 1 1", 'RNGSeed': seed if seed is not None else -521573, 'Step': 0.005, 'Time': time, 
              'SamplePeriod': time //200, 'AnalysisPeriod': time //20, 'DensityPeriod': time, 'DisplayPeriod': time //20, 'RestartPeriod': time // 2,
              }
...
def main():
    ...
    frac = np.linspace(1.125,1.200,35)
    
    # Change FOLDER NAME BEFORE ADDING OTHER SEEDS!!!!!!!!!!!!
    np.random.seed(279)
    seeds = np.random.randint(-9999, -1000, size=1)
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    sims = [{'folder': f'lipid6_{frac[i]:.7f}/', 'frac': frac[i], 'seed': seeds[j]} 
